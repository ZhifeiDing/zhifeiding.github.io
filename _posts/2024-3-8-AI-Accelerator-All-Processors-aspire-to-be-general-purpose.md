...
在计算密度和向量单元高利用率之间的权衡， VPU选择了 16-wide。分析表明，如果 16 个通道一次处理 16 个单独的像素，典型像素着色器的利用率为 88%。Nvidia GeForce 8 以类似的方式运行，其标量 SIMD 处理器组织成 32 个一组，执行相同的指令。主要区别在于，在 Larrabee 中，循环控制、缓存管理等操作是与 VPU 并行运行的代码，而不是作为固定功能逻辑实现
...
每个 TPC 有2 个32x32矩阵乘法核Matrix Multiply Core，支持BF16的矩阵乘法，其他操作支持FP32 和 BF16，包括非 GEMM 的向量运算。张量处理核TPC有可以同时读取和写入的双端口SRAM，以及一个可以从内存中读取数据并使用卷积滤波器进行转换以进行矩阵乘法的卷积引擎(Convolution Engine)。数学运算发生在矩阵乘法核Matrix Multiply Core中，流水线支持乘法前的预操作，然后对最终产品进行多次操作。矩阵乘法核同时输出前操作和后操作的结果，可以最大程度地减少通过计算流水线进行连续数据移动的需要。
...
![94.png](/assets/images/ai/94.png)每个ICE单元有4MB的SRAM，以帮助减少芯片内数据移动，这在功耗和时间方面总是比实际的计算操作更昂贵。深度学习计算网格(Deep Learning Compute Grid)是一个张量引擎，每周期可完成4K MAC运算，支持 FP16 和 INT8，也可以支持 INT4、2 和 1，但是不支持 bfloat16；和SRAM以及VP6 DSP通过数据和控制总线互联。DSP用于向量计算，支持 INT8、16、32 和 FP16 的宽 VLIW；Ice Lake 核可以使用 VNNI 运行其他代码。下图展示了ICE单元架构框图：
...
TPC 具有公开的流水线体系结构。每条指令都有一个预定义的延迟，大部分需要四个周期的延迟。TPC 内核中的所有指令都可以预测。每个 VLIW 插槽都以不同的方式预测：
- SPU 和存储插槽仅支持标量预测。
- VPU 和 Load 插槽可以由单个标量值或位数组来预测，从而实现特定向量元素的屏蔽。

预测通过内部函数向 TPC-C 程序员公开。
...
256x256 矩阵加速器
英特尔® Gaudi®2 集成了 Habana 的第四代 Tensor 处理器内核。TPC 是一种通用 VLIW 处理器，宽度为 256B SIMD，支持 FP32、BF16、FP16 和 FP8（E4M3 和 E5M2），此外还支持 INT32、INT16 和 INT8 数据类型。与需要DMA将操作数传入和传出本地SRAM的普通DSP不同，TPC使用先进的微架构技术，公开了无DMA编程模型，大大简化了软件开发。此外，同样先进的微架构允许内核之间无气泡执行，这有效地使 TPC 100% 用于张量处理，即使对于非常短的内核，无论其输入/输出（SRAM 或 HBM）的位置如何。就像 MME 一样，TPC 在处理小张量时也非常有效。
256x256 matrix accelerator
Intel® Gaudi®2 integrates Habana’s fourth generation Tensor Processor Core. The TPC is a general purpose VLIW processor which is 256B SIMD wide and supports FP32, BF16, FP16 & FP8 (Both E4M3 and E5M2), in addition to INT32, INT16 & INT8 data types. As opposed to common DSPs, which require a DMA to fetch in and out the operands to a local SRAM, the TPC, using advanced micro-architectural techniques, exposes a DMA-free programming model which significantly eases SW development. In addition, the same advanced microarchitecture allows bubble-free execution between kernels which effectively makes TPC 100% utilized on tensor processing, even for very short kernels, regardless of the location of its inputs/outputs (SRAM or HBM). Just like MME, TPC is also very efficient in working on small tensors.
#### 系统扩展
英特尔®高迪®2 AI 加速器 HLBA-225 基板
HLBA-225 是另一款受 OCP 启发的产品，用于简化基于英特尔 Gaudi2 的系统设计人员的系统设计。它支持 8 个英特尔 Gaudi2 夹层卡，这些夹层卡以非阻塞的多对多配置在其 PCB 上被动互连，从每张卡使用 21 个 NIC（3 个端口到 7 个卡中的其他端口），以及将剩余的 3 个 NIC 从每个英特尔 Gaudi2 AI 加速卡 （3x8=24） 路由到 6 个板载 QSFP-DD 连接器， 用于横向扩展。
Intel® Gaudi®2 AI Accelerator HLBA-225 Baseboard
The HLBA-225 is another product, inspired by OCP, and offered for simplifying system design for Intel Gaudi2 -based system designers. It supports eight Intel Gaudi2 mezzanine cards which are passively interconnected on its PCB in a non-blocking all-to-all configuration, using from each card 21 NICs (3 ports to every other of the 7 cards), as well as routing the 3 remaining NICs from every Intel Gaudi2 AI accelerator card (3x8=24) to the six on-board QSFP-DD connectors, for scaling-out.
![107.png](/assets/images/ai/107.png)


### Goya
异构计算架构
• 3 种发动机：TPC、GEMM 和 DMA
• 使用共享SRAM并发工作
张量处理器核心 （TPCTM）
• VLIW SIMD 矢量内核
• C 可编程
混合精度数据类型：FP32、INT32、INT16、INT8、UINT32、UINT16、UINT8
Heterogenous compute architecture
• 3 Engines: TPC, GEMM and DMA
• Work concurrently using a shared SRAM
Tensor Processor Core (TPCTM)
